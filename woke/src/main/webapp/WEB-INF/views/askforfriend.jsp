...
<body>
 	<div class="inner">
		...
		<div class="infos">
			<form method="post" action="<%=path%>/showfriends">
				<div id="entry">
					<div class="form ">
						<label for="qq"></label>
						<div class="item-ifo">
			...
			<div class="right">  
				...
				<c:if test="${!empty users }">
					<c:forEach items="${users}" var="user" varStatus="status">
						<div class="qquser">
							<img src="../headpic/${user.headpic}" width="80" height="80"/> <br>昵称：${user1.username} <br>QQ：${user1.qq}
							<form action="<%=path %>/user/askforFriend" method="post" >
								<input type="hidden" name="friendid" value="${user1.userid }"> 
					...
					<div class="pagelistbox">
						...
						<c:if test="${!empty users }">
							<span> <c:if test="${pager.isFirstPage==false}">
									<a
										href="<%=path%>/showfriends?userid=${user.userid }&currentPage=1">首页</a>
								</c:if> <c:if test="${pager.hasPreviousPage==true}">
									<a
										href="<%=path%>/showfriends?userid=${user.userid }&currentPage=${pager.pageNum-1}">上一页</a>
								</c:if> <c:if test="${pager.hasNextPage==true}">
									<a
										href="<%=path%>/showfriends?userid=${user.userid }&currentPage=${pager.pageNum+1}">下一页</a>
								</c:if> <c:if test="${pager.isLastPage ==false}">
									<a
										href="<%=path%>/showfriends?userid=${user.userid }&currentPage=${pager.pages}">尾页</a>
