	...
	<div class="layout-body">
		<div class="w960  clf">
			<div class="m1_main">
				<div class="inner">
					<div class="hd">
						<h3>${receiverid.username }的留言本</h3>
					</div>
<%-- 					 <c:set var="messages" value="${allmessages.getList()}" />
								...
								<div class="c_list clf">
									...
									<div class="c_fr">
										<p class="c_meta">
											...
											<span class="c_time">${message.message.messagetime }</span>
											<span class="c_time"> #${totalmessages-(pager.pageSize)*(pager.pageNum-1)-status.index}</span>
											<span class="c_time">
												<c:if test="${user1.userid==message.message.senderid }">
											  	   <a href="<%=path %>/deleteMessage?messageid=${message.message.messageid}&receiveid=${receiver.userid }&currentPage=${pager.pageNum }" onclick="return operaconfirm('删除');">删除</a>
												</c:if>
											</span>
										</p>
				...
				<div class="inner">
					...
					<div class="quote_f">
						<form action="<%=path %>/message1" method="post">
							<input name="senderid" value="${user1.userid }" type="hidden">
