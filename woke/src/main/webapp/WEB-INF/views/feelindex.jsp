	...
	<!--qq end-->
 	...
 	<div class="msgCon">
		...
		<c:if test="${!empty feels }"> --%>
			<c:forEach items="${feelList}" var="feel" >
				<div class='msgBox'>
					<dl>
						<dt>
						...
						<dd>
							${feel.user.username } <span style="font-size: 7px;">${feel.feel.feeltime }</span>
							<c:if test="${user1.userid==feel.user.userid }">
								<a
									href="<%=path %>/deleteFeel?fid=${feel.feel.fid}&userid=${feel.user.userid }&currentPage=${pager.pageNum }">
				...
				<div class="pagelistbox">
					<p>
						<span> <c:if test="${pager.isFirstPage==false}">
								<a
									href="<%=path%>/feelindex/${view }?userid=${user.userid}&currentPage=1">首页</a>
							</c:if> <c:if test="${pager.hasPreviousPage==true}">
								<a
									href="<%=path%>/feelindex/${view }?userid=${b}&currentPage=${b-1}">上一页</a>
							</c:if> <c:if test="${pager.hasNextPage==true}">
								<a
									href="<%=path%>/feelindex/${view }?userid=${b}&currentPage=${b+1}">下一页</a>
							</c:if> <c:if test="${pager.isLastPage ==false}">
								<a
									href="<%=path%>/feelindex/${view }?userid=${a}&currentPage=${a}">尾页</a>
							</c:if> 当前第 ${b}页, 总共${a} 页
						</span>
					</p>
				</div>
